...
- вызов tool для запуска основного пайплайна;
...
- семантической нормализации пользовательского запроса в структурированный профиль;
- генерации финального ответа на русском языке;
- follow-up поведения поверх сохраненного session state.

### Deterministic core
Критичная логика не делегируется LLM.
...
Использован **tool-based coordinator pattern**.
...
5. Tool:
   - строит профиль сценария;
   - загружает каталог моделей MWS;
   - рассчитывает стоимость;
   - подбирает модели;
   - формирует структурированный отчет
6. Агент формирует финальный текстовый ответ
